...
def get_word_list() -> list[str]:
    ...
    return list(
        set(map(lambda x: x.strip().upper(), open("words_very_short.txt", "r").readlines()))
    )
...
def make_guess(guess: str) -> bool:
    ...
    for position, letter in enumerate(guess):
        if (
            ...
            raise ValueError(
                f"You have guessed that the letter {letter} should be in position {position} but a previous guess already was flagged as correct indicating that letter {__last_target_word[position]} should be in this position."
            )
    ...
    for position, code in enumerate(__known_pattern):
        if code == IN_WORD and __last_target_word[position] not in guess:
            raise ValueError(
                f"Previous guesses have indicated that letter {__last_target_word[position]} is in the word somewhere, but you have not guessed letter anywhere in your word."
            )
    return False
